...
\begin{document}
    ...
    A simulation study was performed in order to verify the possible usage of the presented model in real life situations, namely on multiple processes with relatively few events (i.e. multiple short walks of the same kind{\color{red})}.
    ...
    Four different task{\color{red}s} were considered:
    \begin{enumerate}
    ...
    To evaluate task~4, {\color{red}all} four presented models {\color{red}were} fitted on the set of $K$ walks and the \emph{AIC} was computed {\color{red}for each one of them}.
    ...
    For $K=100$, $\alpha=0.1$ {\color{red}and all combinations of input parameters and walk lengths $\tilde{\lambda}$, $p_0$ and $n$}, $92\%$ of all evaluation criteria (for tasks~1--3) were successful and the correct model was found in $85\%$ of cases (task~4).
    ...
    Full results of all evaluation setups{\color{red}, i.e. combinations of input parameters $\tilde{\lambda}$ and $p_0$, number of observed walks $K$ and their different lengths $n$} as well as several values of parameter $\alpha$ can be found in the GitHub repository (see the last paragraph of the paper).
