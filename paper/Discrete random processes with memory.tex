...
\author{\|TomÃ¡\'{L}¡ |Kou\'{L}™im|, \|Petr |Volf|, Prague}
...
introduced by Kou\'{L}™im \cite{ja2017ddny, ja2019teze}.
...
Davis and Liu (2016) contains a rather broad definition of such a
...
$p={1\over 2}$. It is also the stationary distribution of the
chain $X_t$.
\end{cor}
...
Then the limit distribution follows from Proposition 2.2 or also
from the fact that $EP_t$ tends to {1\over 2}.

Let $EP_{t-1}={1\over 2}$ be the characteristics of $X_t$, i.e.
$EX_t=0$. As then
$EP_t=EP_{t-1}\lambda+(1-\lambda)/2(1-EX_t)={1\over 2}$, therefore
$EX_{t+1}=0$ again.
