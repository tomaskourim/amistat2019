...
\begin{document}
    ...
    \begin{enumerate}
        \item find {\color{red}$\lambda$ or $[\lambda_{0},\,\lambda_{1}]$ (further denoted as $\tilde{\lambda}$)} with known $p_{0}$ and model type,
    ...
    {\color{red}To evaluate task~4, each of the four presented models was fitted on the set of $K$ walks and the \emph{AIC} was computed.
    The model with the lowest \emph{AIC} value was then selected.
    The quality of such estimation for the given walk configuration was then assesed using the proportion of the number of correctly chosen models to the number of analyzed walk sets $N$}.
    ...
    As expected, the results are less convincing for $K=5$, with only $\color{red}72\%$ of all evaluation criteria being successful and $70\%$ of correctly found models.
    ...
    \begin{table}
        \begin{centering}
            ...
            The mean {\color{red} of parameter estimates} and its standard deviation and the median {\color{red} of parameter estimates} and the corresponding ``percentile'' interval are presented.
