...
\begin{document}
    ...
    Model parameters were then estimated from these sets of K walks, specifically following tasks were performed:
    ...
    An example of the parameter estimation evaluation (tasks 1--3) can be seen in Table~\ref{tab:Fitting-results}, where $\lambda$ or $\lambda_{0}$ with true value $0.5$ (and corresponding $\lambda_{1}=0.8$) was estimated, with known $p_0=0.5$.
    The mean value and its standard deviation and the median and the corresponding ``percentile'' interval are presented.
    An example of the model type identification results (task 4) can be observed in Table~\ref{tab:Fitting-results-model}.
