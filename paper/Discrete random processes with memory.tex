...
\paragraph{\bf Remark:} Naturally, when the initial probability
$p_0=\frac{1}{2}$, then $X_t,\,t=1,2,...$ is the stationary
random sequence with the distribution given in Corollary
\ref{cor-stac-dist}.

Further, it is easy to show that the same holds for
$EX_t$ from $t\ge2$ also when the parameter $\lambda = \frac{1}{2}$.
