...
Basic properties of the walk were also described in these previous
papers. Namely, the value of a transition probability $P_{t+k}$ at
...
\begin{rem}\label{rem:success_pun_varS}
...
of a \emph{success punishing} version of the random walk and its observed
variance (dot-dashed lines, ``thinner''). The values were
computed from 1000 simulated realizations of each parameter combination.}
 \end{center}
\end{figure}
...
of an event is decreased every time event occurs. Opposite situation
...
\emph{success rewarding} model. The \emph{success rewarding} version of the model behaves differently
