...
\begin{document}
    ...
    problems~\cite{schutz2004elephants,turban2010random}.
    Yet there are
    ...
    life events.
    One of such modifications is the random walk with varying
    ...
    {\color{red}A simulation study was performed in order to verify the possible usage of the presented model in real life situations, namely on} processes with relatively few events (i.e. short walks), which can be (under certain assumptions) observed repetitively.
    {\color{red}Such processes include for example} the recurrence of diseases (few recurrences but many patients), reliability of machines (few failures but multiple same machines) or the modelling of sports (few significant events in a match but multiple matches).
    {\color{red}The experiment consisted in generating $K$ random walks of length $n$, of the same walk type and parameter configuration, and estimating the walk type and parameter values from the generated data.
    Four different task were considered:
        $(1)$ find $\tilde{\lambda}$ with known $p_{0}$ and model type,
        $(2)$ find $p_{0}$ with known $\tilde{\lambda}$ and model type,
        $(3)$ find $p_{0}$ and $\tilde{\lambda}$ with known model type and
        $(4)$ find model type without any prior knowledge.
    ...
    The derivation of the theoretical likelihood values is rather complicated, therefore numerical approach {\color{red}using the Python programming language and its scientific package SciPy was applied}.
    ...
    To assess the quality of the parameter estimation {\color{red}(tasks~1--3)} four different evaluation criteria were tested.
    ...
    {\color{red}Finally}, a ``proximity'' interval was constructed based on the true parameter value {$\omega$ as $[\omega-\frac{\alpha}{2}\omega,\,\omega+\frac{\alpha}{2}\omega]$}.
    {\color{red}The third test was positive if the mean fitted parameter value fell into that interval and the fourth test checked whether the median fitted parameter value was within the ``proximity'' interval.}
    {\color{red}For task~4, the model was estimated using the \emph{AIC} on each set of $K$ walks.
    To evaluate the quality of such estimation} the proportion of correctly {\color{red}chosen} model types for the given walk configuration was computed {\color{red}as $\frac{\#\,correctly\,chosen}{N}$}.
    ...
    Longer walks show  {\color{red}generally} better results when finding the coefficients $\tilde{\lambda}$ while the performance of finding correct $p_0$ seems independent on the walk's length.
    ...
    {\color{red}As expected, tasks~1--2 show better results than task~3, as there are less parameters to estimate.}
    An example of the parameter estimation evaluation (tasks~1--3) can be seen in Table~\ref{tab:Fitting-results}, {\color{red}an example of the model type identification results (task~4) can be observed in Table~\ref{tab:Fitting-results-model}.}
    {\color{red}Both tables contain only a brief illustration of results due to space limitations.
    ...
    \begin{table}
        \begin{centering}
        {\caption{\label{tab:Fitting-results} The table shows an example of {\color{red} task $1$} evaluation results{\color{red},
        with $\lambda=0.5$ or $\lambda_{0}=0.5$ (and corresponding $\lambda_{1}=0.8$), $p_0=0.5$, $\alpha=0.1$.
        The mean value and its standard deviation and the median and the corresponding ``percentile'' interval are presented.}
    ...
    A drawback of the presented simulation experiment is the discrepancy between the theoretical model (where $0<P_t<1$, for all $t$) and its representation in computer simulation, where the limited precision {\color{red}rounds values very close to $1$}.
    This rounding causes the optimization algorithm to produce inaccurate estimates or to not converge at all.
    It affects only the \emph{success rewarding} version of the model {\color{red}when estimating the} $\lambda$ parameter and only cases with $K=100$ and $n\geq50$ ($63\%$ not converged estimations){\color{red}, independent of the choice of $\lambda$ or $p_0$ parameters.
    ...
    \begin{table}
        ...
        {\caption{\label{tab:Fitting-results-model}The table shows {\color{red}model estimation} success rate.
            {\color{red}Notation and parameter configuration is the same as in Table~\ref{tab:Fitting-results}.}
        }
