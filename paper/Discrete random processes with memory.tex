...
\begin{document}
    ...
    Longer walks show generally better results when finding the coefficients $\tilde{\lambda}$ {\color{red}especially for the \emph{success rewarding} version of the model (as seen for example in row 10 in Table~\ref{tab:Fitting-results}),} while the performance of finding correct $p_0$ seems independent on the walk's length.
